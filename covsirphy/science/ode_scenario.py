...
class ODEScenario(Term):
    ...
    def __init__(self, data: pd.DataFrame, location_name: str,
                 complement: bool = True) -> None:
        ...
        engineer.clean()
        engineer.transform()
        # DataEngineer.subset returns tuple (subset_df, endpoint_dict) or
        # similar.
        res = engineer.subset(
            geo=self._location_name, variables=[self.S, self.CI, self.R, self.F], complement=complement)
        self._actual_df = res[0]
        self._first: pd.Timestamp = cast(
            pd.Timestamp, self._actual_df.index.min())
        self._last: pd.Timestamp = cast(
            pd.Timestamp, self._actual_df.index.max())
    ...
    def build_with_dynamics(self, name: str,
                            dynamics: Dynamics) -> "ODEScenario":
        ...
        snl_dict = {self.ODE: dyn.model, self.TAU: dyn.tau,
                    self._PARAM: dyn.track().loc[:, dyn.model._PARAMETERS]}
        self._snr_alias.update(name=name, target=snl_dict)
        return self

    def build_with_model(self, name: str, model: type[ODEModel], date_range: tuple[str |
                         None, str | None] | None = None, tau: int | None = None) -> "ODEScenario":
        ...
        if not (isinstance(model, type) and issubclass(model, ODEModel)):
            raise UnExpectedTypeError(
                name="model", target=model, expected=ODEModel)
        ...
        start_date, end_date = Validator(
            ...
            default=(
                None, None), length=2)
        start = Validator(
            start_date, name="the first value of @date_range").date(default=self._first)
        end = Validator(
    ...
    @classmethod
    def auto_build(cls, geo: tuple[list[str] | tuple[str, ...] | str, ...] | str |
                   None, model: type[ODEModel], complement: bool = True) -> "ODEScenario":
        ...
        engineer.download(
            country=geo[0] if isinstance(
                geo, (tuple, list)) and len(geo) > 1 else None,
            province=geo[1] if isinstance(
                geo, (tuple, list)) and len(geo) > 2 else None,
        ...
        # Create instance
        snl = cls(data=subset_df.loc[subset_df[cls.R] > 0], location_name=GIS.area_name(
            geo=geo), complement=False)
    ...
    def delete(self, pattern: str, exact: bool = False) -> "ODEScenario":
        ...
        names = [name for name in self._snr_alias.all().keys()
                 if p.search(name)]
        for name in names:
    ...
    def to_dynamics(self, name: str) -> Dynamics:
        ...
        model, tau, param_df = [temp_snl_dict[k]
                                for k in [self.ODE, self.TAU, self._PARAM]]
    ...
    def summary(self) -> pd.DataFrame:
        ...
        return pd.concat(dataframes, axis=0).set_index(
            [self.SERIES, self.PHASE]).convert_dtypes()
    ...
    def simulate(self, name: str | None = None,
                 variables: list[str] | None = None, display: bool = True, **kwargs: Any) -> pd.DataFrame:
        ...
        if name is None:
            ...
            title = f"{
                self._location_name} ({name} scenario): simulated number of cases over time"
            v = dyn.start_dates()[1:]
        sirf_df[self.SERIES] = name or self.ACTUAL
        engineer = DataEngineer(layers=[self.SERIES])
        engineer.register(data=sirf_df.reset_index())
        engineer.inverse_transform()
        v_converted = self._variable_alias.find(
            name=variables, default=[self.C, self.F, self.R])
        df = engineer.all().set_index(
            self.DATE).loc[self._first:self._last, v_converted]
        # Show figure
        if display:
            plot_kwargs = {
                "title": title,
    ...
    def compare_cases(self, variable: str, date_range: tuple[str | None, str | None] | None = None,
        ...
        if v_converted == self.C:
            df[self.ACTUAL] = self._actual_df[[
                self.CI, self.F, self.R]].sum(axis=1)
        elif v_converted == self.N:
            df[self.ACTUAL] = self._actual_df[[
                self.CI, self.F, self.R, self.S]].sum(axis=1)
        else:
        ...
        start_date, end_date = Validator(
            ...
            default=(
                None, None), length=2)
        start = Validator(
            start_date, name="the first value of @date_range").date(default=df.index.min())
        end = Validator(
        ...
        if display:
            ...
            v = self.to_dynamics(
                name=ref or list(
                    self._snr_alias.all().keys())[0]).start_dates()[
                1:]
            plot_kwargs = {
                "title": title,
                "y_integer": True,
    ...
    def compare_param(self, param: str, date_range: tuple[str | None, str | None] | None = None,
        ...
        if display:
            ylabel, h = (
                self.RT_FULL, 1.0) if param == self.RT else (
                param, None)
            title = f"{self._location_name}: {ylabel} overt time"
            v = self.to_dynamics(
                name=ref or list(
    ...
    def _append(self, name: str, end: pd.Timestamp |
                int, **kwargs: Any) -> None:
        ...
        if isinstance(end, int):
            ...
            # Assume Timestamp
            try:
                # Check if it behaves like a timestamp (has year, month, etc)
                # or use Validator
                end_date = Validator(end, "end", accept_none=False).date(
                    value_range=(param_df.index[-1], None))
            except (UnExpectedTypeError, AttributeError):
                # Fallback if it was a string or something else that looks like
                # int but wasn't caught
                try:
                    delta = timedelta(
                        days=Validator(
                            end, "end", accept_none=False).int(
                    ...
                    raise UnExpectedTypeError(
                        name="end", target=end, expected=pd.Timestamp) from None
        ...
        try:
            ...
            if isinstance(end, int):
                ...
                _min = (param_df.index[-1] + timedelta(days=3)
            ...
            raise UnExpectedValueRangeError(
                name="end", target=target, value_range=range_val) from None
        self._snr_alias.update(name=name, target=snr_dict.copy())
        self._last = max((self._last, end_date))

    def append(self, name: str | list[str] | None = None, end: pd.Timestamp |
               int | None = None, **kwargs: Any) -> "ODEScenario":
        """Append a new phase, specifying ODE parameter values.
    ...
    def predict(self, days: int, name: str, seed: int | None = 0, verbose: int = 1,
                X: pd.DataFrame | None = None, **kwargs: Any) -> "ODEScenario":
        ...
        param_df["Phase"] = (param_df[self.RT] !=
                             param_df[self.RT].shift()).cumsum()
        phase_df = param_df.groupby("Phase").last().drop(
            self.RT, axis=1).rename(columns={self.DATE: "end"})
    ...
    def represent(self, q: list[float] | float, variable: str, date: str | None = None,
        ...
        """
        quantiles = [q] if isinstance(
            q, float) else Validator(
            q, "q").sequence()
        v_converted = self._variable_alias.find(
            name=variable, default=[variable])
        Validator(
            v_converted,
            "variable",
            accept_none=False).sequence(
        ...
        in_set = all_set if included is None else set(
            Validator(included, "included").sequence())
        ex_set = set() if excluded is None else set(
            Validator(excluded, "excluded").sequence())
        scenarios = list(all_set & (in_set) - ex_set)
        # Get simulation data of the variable of the target scenarios
        sim_dict = {name: self.simulate(name=name, display=False)[
            variable] for name in scenarios}
