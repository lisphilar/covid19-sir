...
class _DataTransformer(Term):
    ...
    def infected(self, new, **kwargs):
        ...
        df = Validator(
            ...
            columns=list(
                c_dict.values()))
        df[new] = df[c_dict["confirmed"]] - \
            df[c_dict["fatal"]] - df[c_dict["recovered"]]
        self._df = df.copy()
    ...
    def diff(self, column, suffix, freq):
        ...
        new_df = df.set_index(
            ...
            observed=True).shift(
            freq=freq)
        new_df = new_df.loc[:, [column, *self._layers]
    ...
    def sub(self, minuend, subtrahend, new, fill_value):
        ...
        Args:
            ...
            fill_value (float): value to fill in NAs
        """
        v = Validator([minuend, subtrahend],
                      "columns of numerator and subtrahend")
        v.sequence(candidates=list(self._df.columns))
        df = self._df.copy()
        df[new] = df[minuend].sub(
            df[subtrahend], fill_value=Validator(
                fill_value, "fill_value").float())
        self._df = df.copy()
    ...
    def div(self, numerator, denominator, new, fill_value):
        ...
        """
        v = Validator([numerator, denominator],
                      "columns of numerator and denominator")
        v.sequence(candidates=list(self._df.columns))
        df = self._df.copy()
        df[new] = df[numerator].div(
            df[denominator], fill_value=Validator(
                fill_value, "fill_value").float())
        self._df = df.copy()
