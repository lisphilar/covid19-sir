...
class ODEModel(Term):
    ...
    def __init__(self, date_range: tuple[str, str], tau: int,
        ...
        self._tau: int = Validator(
            tau, "tau", accept_none=False).tau()  # type: ignore
    ...
    def settings(self, with_estimation: bool = False) -> dict[str, Any]:
        ...
        if with_estimation:
            _dict["estimation_dict"] = self._estimation_dict or {
                "method": "not_performed"}
        return _dict

    @classmethod
    def from_sample(
            cls, date_range: tuple[str | None, str | None] | None = None, tau: int = 1440) -> Self:
        ...
        start_date, end_date = Validator(
            ...
            default=(
                None, None), length=2)
        start = Validator(
            start_date, name="the first value of @date_range").date(default=datetime.now())
        end = Validator(
    ...
    def solve(self) -> pd.DataFrame:
        ...
        """
        step_n = math.ceil((self._end - self._start) /
                           timedelta(minutes=self._tau))
        sol = solve_ivp(
            fun=self._discretize,
            t_span=[0, step_n],
            y0=np.array([self._initial_dict[variable]
                        for variable in self._VARIABLES]),
            t_eval=np.arange(0, step_n + 1, 1),
            dense_output=False
        )
        df = pd.DataFrame(data=sol["y"].T.copy(), columns=self._VARIABLES)
        df = self._non_dim_to_date(
            data=df, tau=self._tau, start_date=self._start)
        return df.round().convert_dtypes()

    @staticmethod
    def _date_to_non_dim(series: pd.DatetimeIndex,
                         tau: int | None) -> pd.DatetimeIndex:
    ...
    @classmethod
    def _non_dim_to_date(cls, data: pd.DataFrame, tau: int,
                         start_date: str | pd.TimeStamp | None) -> pd.DataFrame:
        ...
        if (tau is None and start_date is None) or isinstance(
        ...
        if tau is None:
            raise NotNoneError(
                "start_date",
                start_date,
                details="None is required because tau is None")
        ...
        df[cls.DATE] = (start + pd.Series(df.index *
                        tau).apply(lambda x: timedelta(minutes=x))).tolist()
    ...
    def inverse_transform(cls, data: pd.DataFrame, tau: int | None = None,
                          start_date: str | pd.Timestamp | None = None) -> NoReturn:
    ...
    def from_data(cls, data: pd.DataFrame,
                  param_dict: dict[str, float], tau: int = 1440, digits: int | None = None) -> Self:
    ...
    @classmethod
    def from_data_with_quantile(cls, data: pd.DataFrame, tau: int = 1440,
                                q: float = 0.5, digits: int | None = None) -> Self:
        ...
        trans_df = cls.transform(data=data.set_index(cls.DATE), tau=tau)
        cls_obj = cls.from_data(
            data=data, param_dict=cls._param_quantile(
                data=trans_df, q=q), tau=tau, digits=digits)
        cls_obj._estimation_dict = dict(
            method="with_quantile", tau=tau, q=q, digits=digits)
        return cls_obj

    @classmethod
    def _param_quantile(cls, data: pd.DataFrame, q: float |
                        pd.Series = 0.5) -> NoReturn:
    ...
    @classmethod
    def _clip(cls, values: float | pd.Series, lower: float,
              upper: float) -> float | pd.Series:
        """
        ...
        Returns:
            clipped array
        """
        return min(max(values, lower), upper) if isinstance(
            values, float) else pd.Series(values).clip()

    @classmethod
    def from_data_with_optimization(cls, data: pd.DataFrame, tau: int = 1440,
                                    metric: str = "RMSLE", digits: int | None = None, **kwargs) -> Self:
    ...
    @classmethod
    def _estimate_params(cls, data: pd.DataFrame, tau: int, metric: str,
                         **kwargs) -> tuple[dict[str, float], float, str, int]:
        ...
        pruner = pruner_class(**Validator(kwargs,
                                          "keyword arguments").kwargs(pruner_class))
        ...
        for _ in range(iter_n):
            ...
            study.optimize(
                objective_func,
                n_jobs=1,
                timeout=kwargs["timeout_iteration"])
    ...
    @classmethod
    def _optuna_objective(cls, trial: optuna.Trial, data: pd.DataFrame,
                          tau: int, value_range_dict: dict[str, pd.Series], metric: str) -> float:
    ...
    @classmethod
    def _optuna_score(
            cls, param_dict: dict[str, float], data: pd.DataFrame, tau: int, metric: str) -> float:
        ...
        model = cls.from_data(
            data=cls.inverse_transform(df).reset_index(),
            param_dict=param_dict,
            tau=tau,
            digits=None)
        sim_df = model.solve()
    ...
    @classmethod
    def _optuna_is_in_allowance(
            cls, param_dict: dict[str, float], data: pd.DataFrame, tau: int, allowance: tuple[float, float]) -> bool:
        """
        ...
        """
        df = cls.inverse_transform(
            data=data, tau=tau, start_date="01Jan2022").reset_index()
